...
def shapes(item_id, name, category, price, category_id=None):
    """Payload shapes. Schema derived from successive 422s, Aug 21 2026.

    Confirmed by error progression:
      body is a BARE ARRAY          [ {...} ]      (wrapped forms -> 400)
      name required                 ERR_LINE_ITEM_NAME_REQUIRED
      unit_price_decimal, decimal   ERR_LINE_ITEM_UNIT_PRICE_INVALID
      category is a NESTED OBJECT   line_items[0].category.id is required
                                    (flat category_id does NOT satisfy it)

    So the write schema mirrors the read schema: *_decimal strings and
    nested {category: {id}}. Line items are created BY VALUE -- the
    singular POST /line-items documents no item_id at all.
    """
    p = f"{float(price or 0):.2f}"
    base = {"name": name, "quantity_decimal": "1", "unit_price_decimal": p,
            "category": {"id": category_id}}
    return [
        ("nested category",       [dict(base)]),
        ("+ item_id",             [dict(base, item_id=item_id)]),
        ("+ cost/taxable",        [dict(base, unit_cost_decimal="0",
                                        taxable=True)]),
    ]
        ...
        if isinstance(o, dict):
            ...
            for k, v in o.items():
                if k == "item_id":
                    ...
                    out[k] = bundle["name"]
                elif k == "id" and key == "category":
                    out[k] = bundle.get("category_id")
                else:
                    out[k] = swap(v, k)
            return out
        if isinstance(o, list):
            return [swap(x, key) for x in o]
        return o
