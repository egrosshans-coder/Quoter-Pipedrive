...
def shapes(item_id, name, category, price, category_id=None):
    """Candidate payloads — narrowing by evidence, not by guessing.

    CONFIRMED so far (Aug 21 2026):
      * body is a BARE JSON ARRAY   -> [ {...} ]  parsed; wrapped forms 400
      * `name` is required          -> ERR_LINE_ITEM_NAME_REQUIRED
      * price field is `unit_price_decimal`, and must be a non-negative
        DECIMAL STRING              -> ERR_LINE_ITEM_UNIT_PRICE_INVALID
      * `category` as a plain string caused 400 format-invalid; in the read
        schema category is an OBJECT, so it is likely category_id or omitted.

    Write fields therefore mirror the read schema: *_decimal strings.
    """
    p = f"{float(price or 0):.2f}"
    base = {"name": name, "quantity_decimal": "1", "unit_price_decimal": p}
    return [
        ("minimal decimals",      [dict(base)]),
        ("+ category_id",         [dict(base, category_id=category_id)]),
        ("+ item_id",             [dict(base, item_id=item_id)]),
        ("+ unit_cost + taxable", [dict(base, unit_cost_decimal="0",
                                        taxable=True)]),
    ]
    ...
    def swap(o):
        if isinstance(o, dict):
            return {k: (bundle["id"] if k == "item_id"
                        ...
                        else bundle.get("category_id") if k == "category_id"
                        else swap(v))
                    for k, v in o.items()}
