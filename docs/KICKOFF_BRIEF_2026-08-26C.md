...
**The client-facing quote is done.** Scope of Work leads, then the selected effects, then shipping and travel. Line item descriptions render, quote numbers use the TLC format, and section names carry no internal prefix.
...
| **Auto-append** | `SCO-ScopeOfWork` is added to every quote as the first section and hidden from the dropdown |
...
Naming: **`SCO-`** scope · **`SFX-`** effects · **`SVC-`** generic services · **`STE-`** shipping/travel/expenses. Group name and Pipedrive label must stay identical — the sync matches on them. The client-facing section name comes from `item_group_defs.json` and is free.

**`SCO-ScopeOfWork` is auto-appended** to every quote as the first section and deliberately **excluded from the Pipedrive dropdown** — offering it would let a salesperson produce two Scope sections. Marked `auto_append: true` in the defs; the composer prepends and de-duplicates, the sync skips it. §13.5.

**Quote numbering is unconditional.** `ENABLE_CUSTOM_NUMBER_PATCH` is gone — it existed because legacy could not set a number reliably, and v2 sets `custom_number` at create. Format `dealID-YYYYMMDD`. §13.6.
...
### 4.5 Section subtotals cannot be separated from the split

A per-section subtotal renders **only** with *Separate One-time/Recurring Prices* on, and it brings a "One-Time Fees" header. There is no combination giving line items without prices AND a section subtotal.

**This matters less than it looks.** TLC bundles — "$5,000 gets you balloons and Floating Video" — so the price is not per-section anyway. What is wanted is one number for the quote with sections showing scope, which the current configuration produces. **[Open]** whether that number sits on the `Scope of Project` line. Chapter 3 §14.3.

### 4.6 Lowercase `x-api-key` is mandatory
...
**Line item descriptions render; catalog item descriptions do not.** Two different fields. The composer copies one to the other — that one line is what makes catalog wording reach the quote.

**Pruning: blank the quantity, do not zero it.** `1` = priced line, `0` = line at $0.00, **blank** = line removed. **Backspace does not clear the field; the delete key does** — a salesperson using backspace leaves a line they think they deleted.

**"Cost Breakdown" shows PRICE, not cost**, despite the name. Confirmed with cost 1000 / price 600: it reported 600. Safe client-facing.

**The "Summary" block appears automatically** on any quote with more than one section — so on every composed quote. Structural, not removable; only its header text is editable, in Localization.
...
| `quote_composer.py` | **the v2 quote path** — deal in, quote out. No dependency on legacy `quoter.py` |
...
**[Open]** Where the bundled price sits — the `Scope of Project` line is the obvious home, keeping one price on the line describing the whole project with everything below at $0.00. Cost tracking is unaffected either way; unit costs stay on the line items. §14.3.

**[Open]** Whether to rename the Summary block to "Notes" in Localization. It appears automatically on any multi-section quote, so on every composed quote, and cannot be moved. §14.5.

**Cleanup** — Chapter 3 §16 has the full list. Now also includes the deal-3101 test quotes and contact `3101@gmail.com`.
