...
**Date:** 2026-08-27
**Supersedes:** all prior Chapter 3 versions (2026-08-18-C/D/E/F/G, 2026-08-21-B/C/D/E/F/G/H, 2026-08-23, 2026-08-24, 2026-08-25, 2026-08-26) and the 2026-08-20 addenda (A/B/C). Those are superseded in full, not amended — several contain claims this version retracts (see §2.2.2, §2.2.7, §2.3.1, §7.8). Archive them; do not hand more than one Chapter 3 to a new session.
...
## 16. Open Questions for ScalePad / Jon Turner
...
- **Rotate `WEBHOOK_SECRET`.** It appears in the request URL and therefore in access logs; it was echoed in a log line during testing. Prefer the `X-Webhook-Token` header over `?token=` afterwards.
- **Demote the retry 404 to a warning** inside `add_line_items_retrying`. It logs at ERROR on every multi-section quote before the retry catches it, so real failures do not stand out (§15.6).
- **Reconcile `render.yaml`** — it declares `plan: free` while the service runs on Starter, and omits `SCALEPAD_API_KEY` from `envVarsFrom` (§15.5).
