...
21 Item Groups now exist, resolved from catalog code prefixes rather than from template contents. §8.2.1 gives the reasoning; this records what was built.
...
### 10.6 `STE-After-Bill` and the description that does not render
...
**[Confirmed] Item descriptions do not render on the client-facing quote.** The description was written to carry the terms — *"Travel and expenses are not included in this quote. Airfare, lodging, meals, per diem, ground transport and shipping will be invoiced at actual cost following the event."* The rendered quote shows only Category, Item, Qty, Price, Total. The client sees `Terms | T&E - Billed After Event | 1 | $0.00`.
...
### 13.5 Not yet carried over from legacy
...
- **Quote numbering.** `generate_sequential_quote_number()` is wired in behind `ENABLE_CUSTOM_NUMBER_PATCH`, still off. v2 sets `custom_number` at create, which legacy could not — so the flag could go away.

---


## 14. Open Questions for ScalePad / Jon Turner
