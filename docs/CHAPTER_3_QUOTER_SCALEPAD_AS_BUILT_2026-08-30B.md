...
**Date:** 2026-08-30 (revision B — supersedes 2026-08-30)
**Supersedes:** all prior Chapter 3 versions (2026-08-18-C/D/E/F/G, 2026-08-21-B/C/D/E/F/G/H, 2026-08-23, 2026-08-24, 2026-08-25, 2026-08-26, 2026-08-27, 2026-08-28) and the 2026-08-20 addenda (A/B/C). Those are superseded in full, not amended — several contain claims this version retracts (see §2.2.2, §2.2.7, §2.3.1, §7.8). Archive them; do not hand more than one Chapter 3 to a new session.
...
## 15. Production: the Webhook Branch — **[Confirmed live, 2026-08-27]**

The v2 path now runs in production. A Pipedrive automation fires, Render composes the quote, and every downstream notification fires unchanged.

### 15.1 One call site, behind a flag

`webhook_handler.py` had a single line creating the quote. It is now a branch:

```python
use_v2 = os.getenv("USE_V2_COMPOSITION", "false").lower() in ("true", "1", "yes")

if use_v2:
    full_deal = get_deal_by_id(deal_id)      # see 15.2
...
**Default is false**, so deploying changed nothing. **Reverting is a Render environment setting, not a deploy or a rollback** — which matters on a service with no staging environment.

Everything after the branch is untouched. `if quote_data:` runs either way, so duplicate-marking, Slack, email and the Pipedrive note all work for both paths. The v2 path slotted in without altering anything downstream.

### 15.2 The v2 branch re-fetches the deal, and this is the important part

The webhook builds a **mock** `deal_data` from the three fields the payload happens to carry:

```python
deal_data = {'id': ..., 'title': ..., '42ab0c91...': template_enum_str}
```

**Field 102 (Quote Effects) is not in the webhook payload at all.** Without a re-fetch the composer would find no effects and abort every time.

Two ways to fix that, and the choice matters:

| | |
| :-- | :-- |
| **Widen the payload** | edit the Pipedrive automation to send field 102. Means changing something outside this repo, and the legacy path has to keep working with the new shape. |
| **Re-fetch the deal** | `get_deal_by_id()` returns every custom field. Nothing outside the repo changes. **Chosen.** |

Cost is one extra API call per quote — nothing at TLC's volume. The benefit is that the payload can now **shrink** rather than having to grow: §13.4 established the webhook answers *when* and the API answers *what*, and this is the first place that pays off.

### 15.3 Confirmed run, 2026-08-27 22:08 UTC

Fired against the deployed service with a payload deliberately omitting field 102:

```
👤 Contact 3101@gmail.com: already present
🔢 Quote number: 03101-20260827
✅ Created draft quot_3IW7X5sqOsYm0zP1MJM2c8dKwf9
   + Scope of Work: 1 item(s)
   + Balloon Effects: 8 item(s)
   [ERROR] ScalePad API error 404: ERR_NOT_FOUND
   + Shipping, Travel & Expenses: 1 item(s) (after 2 attempts)
🎉 3 section(s), 10 line item(s), all at $0.00
✅ Marked organization 4351 (deal 3101) as processed
📢 Slack sent to #d-quoter-render-ok
📧 Email sent to admin@ and sales@
📝 Pipedrive note created for deal 3101
```

**Three things this proved that no local run could.** The flag was read from Render's environment. The branch was taken. And **the re-fetch worked** — the payload carried no field 102, yet Quote Effects resolved. That last one had never executed anywhere before this run.

The 404 is §9's eventual consistency, caught and retried. Expected.

### 15.4 The handler's own gates are unchanged

Two conditions stop a webhook before the composer runs. Neither is new, and both return **200 with a reason** — so a 200 does not mean a quote was created:

| Reason | Cause |
| :-- | :-- |
| `not_ready_for_quotes` | HID-QBO-Status is not `289` / `QBO-SubCust` |
| `already_processed` | `processed_organizations.txt` holds `orgID:dealID` |

That second file lives on Render's disk and persists across requests, so a deal used once for testing will not fire again without clearing the line via Render's Shell.

`WEBHOOK_SECRET` is **set**, so the endpoint fails closed. It accepts either `?token=` or an `X-Webhook-Token` header.

### 15.5 Environment

| | |
| :-- | :-- |
| Service | `quoter-webhook-server`, Python 3, **Starter** plan |
| URL | `https://quoter-webhook-server.onrender.com` |
| Deploy | auto on push to `main` |
| Flag | `USE_V2_COMPOSITION` — **currently true** |

**`render.yaml` says `plan: free` but the service runs on Starter.** The blueprint is stale, so a blueprint-driven redeploy could try to downgrade it. Worth reconciling. `SCALEPAD_API_KEY` is likewise set in the dashboard but **not declared in `envVarsFrom`**.

Being on Starter rather than free means no idle spin-down, so the cold-start concern noted earlier does not apply.

### 15.6 Consequences to act on

**The flag is ON.** Any deal reaching the readiness gate now goes through v2. If sales is not ready, set it to `false`.

**The 404 logs at ERROR before the retry catches it**, on every multi-section quote. It will be chased by whoever reads the logs next. Worth demoting to a warning inside `add_line_items_retrying`, so genuine failures stand out.

**`WEBHOOK_SECRET` appeared in a log line** during testing — it is echoed in the request URL. Worth rotating, and worth noting that `?token=` puts the secret in every access log. The header form does not.

---

## 16. Production Hardening — **[Confirmed live, 2026-08-28]**

### 16.1 Flask's development server was running in production

The deploy log carried, on every start:
...
`startCommand` was `python3 webhook_handler.py`, which runs Werkzeug's development server — single-threaded by default, no request queuing, error handling aimed at debugging rather than resilience.

**Now:**

```
gunicorn webhook_handler:app --bind 0.0.0.0:$PORT --workers 2 --timeout 120
```

with `gunicorn==23.0.0` in `requirements.txt`.

**Both flags are deliberate.**

`--timeout 120` because **quote composition takes ten seconds or more** — catalog pull, contact resolution, section creation, then per-section line items with retries. Gunicorn's default is 30 seconds, which would kill a slow quote mid-write and leave a half-built draft.

`--workers 2` is safe **only because `processed_organizations` is file-backed**. Workers are separate processes with separate memory; had that set been a module-level variable, each worker would have kept its own copy and duplicate detection would have silently broken. This was checked before choosing the number, not after.

**Nothing was lost by the switch.** Gunicorn imports the module rather than executing it, so an `if __name__ == "__main__":` block never runs — but this one contains only `app.run()`. The periodic cleanup lives inside the `/health` route, which Render polls, so it continues to fire.

### 16.2 `processed_organizations.txt` does not survive a deploy — **[Confirmed]**

Duplicate protection is a file on Render's disk. After the gunicorn deploy, deal 3101 composed a **new** quote rather than returning `already_processed`, having been processed twice before.

So the file resets on every deploy. In practice this matters little — a deal crosses the stage boundary rarely — but it is **not the guarantee the code implies**, and anyone relying on it should know.

Two smaller notes on the same mechanism. The read and the write are not atomic, so two simultaneous webhooks for one org could both pass the check; unlikely at this volume, and true with one worker as well. And a webhook returns **200 with a reason** when it skips, so a 200 does not mean a quote was created (§15.4).

### 16.3 A recovered 404 no longer logs as an error

Section reads are eventually consistent (§9), and `add_line_items_retrying()` recovers. But `ScalePadV2Client._request()` logged the failure at ERROR *before* raising, so every multi-section quote wrote a red line that had been immediately recovered from — burying the failures that matter.

`_request()` now takes **`expect_statuses`**. A status the caller anticipates logs at WARNING; everything else stays at ERROR. The exception raises either way, so control flow is unchanged. `add_line_items` passes `(404,)`.

**Opt-in and narrow on purpose.** A blanket "404s are fine" rule would hide a mistyped quote id or a deleted section, which is exactly what this logging exists to surface.

**Not a delay.** A longer sleep only makes the race less likely and means picking a number that will be wrong under load (§9.4).

Result:

```
[WARNING] ScalePad API error 404: ... (expected; caller will handle)
   + Shipping, Travel & Expenses: 1 item(s) (after 2 attempts)
```

### 16.4 `render.yaml` is inert, and now accurate anyway

It declared `plan: free` while the service ran **Starter** — which is itself the evidence that Render is *not* syncing from the file. Had it been, the plan would have been downgraded.

So the file is documentation, not configuration. It has been corrected regardless: `plan: starter`, the gunicorn `startCommand`, and the three variables that were set in the dashboard but undeclared — `SCALEPAD_API_KEY` and `WEBHOOK_SECRET` under `envVarsFrom`, `USE_V2_COMPOSITION` under `envVars`.

**`envVars` carries literal values; `envVarsFrom` says "look this up in the secret store."** Putting a `fromSecret` entry in the first, or a `value` in the second, is the easy mistake.

**The start command that actually runs is in Render → Settings → Start Command**, and was changed there too. Worth remembering which of the two is real.

### 16.5 Webhook authentication: Basic Auth, not custom headers — **[Confirmed, not yet implemented]**

`WEBHOOK_SECRET` currently travels as `?token=` in the URL, so it lands in **every access log** — structural, not accidental. It was also echoed in a log line pasted during testing, so it needs rotating regardless.

`_is_authorized()` already accepts an `X-Webhook-Token` header as well, which looked like the answer. **It is not: Pipedrive's automated webhooks do not support custom headers.** They support **HTTP Basic Auth** — username and password fields on the webhook definition — which travels in the `Authorization` header and never appears in a URL.

That is the better answer, and the handler does not support it yet. `request.authorization` exposes it in Flask.

**Sequence, when done:**

1. add Basic Auth to `_is_authorized()`, **keeping** the existing methods so nothing breaks mid-change
2. deploy
3. set username and password on the v3 webhook in Pipedrive, and strip `?token=` from its URL
4. rotate `WEBHOOK_SECRET`; update Render and both local `.env` files
5. test
6. remove the query-token path once nothing uses it

**Also found:** `Ready-Quoter-Draft Quote Creation-v2` still exists, points at the same endpoint, carries **no token**, and is referenced by **4 automations**. If any of them fires it gets a 401. Worth confirming it is genuinely dead before removing.

**Rotation touches four places**, and GitHub Actions is not one of them — the dropdown sync talks to Quoter and Pipedrive directly, never to this endpoint. Order matters: update the callers *before* Render, or every caller breaks until you catch up.

---

## 17. Template Archiving, and the Death of `template_mapping_enhanced.py` — **[Confirmed, 2026-08-30]**

Work done in a parallel Cowork session on 2026-08-29, reconciled here. Its handoff is `docs/WORK_REQUEST_CHAPTER_3_20260829.md`; **two of its claims are corrected below.**

### 17.1 The `USE_V2_COMPOSITION` rollback no longer exists — **[Confirmed]**

§15.1 sells the flag as a free escape hatch: *"Reverting is a Render environment setting, not a deploy or a rollback."*

**That is no longer true.** Ten of the eleven field-90 options were deleted on 2026-08-29, and `template_selection_logic.py` still hard-codes all eleven option ids with no runtime lookup and no default. Setting the flag to `false` now points the legacy path at options and templates that do not exist.

**The v2 path is the only path.** Any plan assuming a one-setting revert needs rewriting. Restoring a rollback means migrating `template_selection_logic.py` to `PipedriveFields.option_map(90)` first — promoted from housekeeping to the thing standing between you and a way back.

### 17.2 What changed in Quoter and Pipedrive

| | |
| :-- | :-- |
| Field 90 options | 13 → **2**. Only `441 Basic` and `528 Standard` remain |
| Deleted | 442, 443, 444, 451–457, plus a leftover `XX-RET-*` from §11.8 testing |
| Quoter templates | ten item-named templates **renamed `YY-<name>`**, not deleted. Content intact |
| Affected deals | **82** stored a deleted option, **33 open**. Their Quote Template field now reads blank |

Composition is unaffected — `quote_composer.py` falls back to `DEFAULT_TEMPLATE_ID` and logs a warning. The 82 were scanned across all 1,772 non-deleted deals before the deletion, not assumed.

### 17.3 The `YY-` archive filter

Renaming alone does not hide a template: `build_plan()` reads it as a label change on the same option id, so `YY-Balloons` stays selectable. Deleting the option alone does not work either — `if oid not in by_id: to_add.append(rec)` re-adds it next run.

**So the exclusion has to happen at fetch time**, mirroring the auto-append exclusion already in `fetch_quoter()`.
...
Plus an `--include-archived` diagnostic flag, which the scheduled run must never pass.

**The prefix taxonomy, now three-way and worth memorising:**

| Prefix | Applies to | Meaning |
| :-- | :-- | :-- |
| `XX-RET-` | Pipedrive option label | retired — record gone, id kept so old deals resolve |
| `YY-` | Quoter template title | archived — kept in Quoter, hidden from the sync |
| `zz-` / `ZZZ-` | Quoter items and quotes | test artifact, safe to delete |

**Un-archiving is a rename.** Drop the `YY-` and the record reappears next run; if its option was retired rather than deleted, `to_unretire` reuses the **same option id** and every deal that stored it resolves again.

**[Confirmed live]** after applying: `excluded 10 archived record(s) [YY-]`, 2 records against 2 options, `ADD 0 / ORPHAN 0`.

**Note the filter applies to item groups too**, since `fetch_quoter()` is shared by both sources. A group named `YY-*` would drop out of field 102 identically. Probably desirable; it was not a deliberate design decision.

### 17.4 The archiving rationale

Deletion in Quoter is permanent — there is no archive. The ten templates hold roughly **202 curated line items** encoding a human judgement about what each kind of job needs, and those judgements were never transferred into `item_group_defs.json`, which derives membership from code prefixes instead (§8.2.1).

ScalePad describes the template-line-item gap as intended (§6.2.1) but the related catalog-reference limitation as one that *"will likely be updated at some point"* (§7.12.0).

**The deciding factor is cost asymmetry, not probability.** Keeping them costs a few lines of filtering. Deleting them costs days of reconstruction if the capability ever lands.

### 17.5 `template_mapping_enhanced.py` is dead — **[Confirmed, and this corrects the work request]**

The file has three parts. All three are now accounted for.

**`TEMPLATE_BUNDLES`** — superseded by Item Groups.

**`appended_content`, 11 variants** — the work request treats these as content needing a home. Reading them says otherwise. They are **a prose restatement of the catalog**:

> *"Robotic Dog: AI-powered robotic companion"* · *"Cryo Jet: High-impact CO2 burst effects"* · *"Dewar Tanks: Tall (180) and New Footprint (230) options"*

Every one already exists as a catalog item with its own description, and line item descriptions render (§14.1). Worse, the block appended the **entire product line** to every quote regardless of what was sold — a client buying one robot dog received a list of every robot TLC owns.

**Seeded line items with descriptions, plus `SCO-ScopeOfWork`, replace this and do it better:** only what is actually on the quote, with text maintained in one place.

**`cover_letter`, 2 variants** — one uses `{{person.first_name}}` and `{{deal.title}}`, **Pipedrive payload syntax Quoter cannot resolve**. The other uses correct `##` mail-merge tokens and is worth keeping (§17.6).

### 17.6 The content never shipped — **[Confirmed, source read]**
...
`get_template_info()` fetches this content and **discards it**. Two consequences, both correcting things assumed elsewhere:

**The `{{...}}` substitution question is moot.** Nothing reached a quote, so no client ever saw a literal `{{person.first_name}}`. There is no live defect in existing documents — the work request's §1.3 can be closed rather than investigated.

**The v2 path lost nothing.** The work request's §1.2 says v2 quotes ship without this content *"and there is no fallback"*, implying legacy had it. **Legacy did not either.** Nothing changed when the flag flipped; this has been dead for months.

**So the file can be retired** once the cover letter text is saved: delete `template_mapping_enhanced.py`, retire `verify_bundles.py` which imports it, and remove the orphaned calls at `quoter.py:1748` and `:2003`.

### 17.7 Cover Page works, and the org name leaks — **[Confirmed live]**

The salvaged cover letter was pasted into `Standard` → **Cover Page** and rendered on a quote. All five tokens resolved:

```
Proposal for Aim Games-3097
Dear Rick,
Thank you for the opportunity to work with Aim Games-3097. This quote was
...
`##CustomerOrganization##` · `##CustomerFirstName##` · `##QuoteCreatedDate##` · `##UserFirstName##` · `##UserLastName##` — all confirmed working.

**Three practical notes.** The Content field is a **rich-text editor with no HTML source view**, so pasting markup prints the tags literally; paste plain text and format with the toolbar. **Title** and **Subtitle** are separate fields above Content and both accept merge tokens. And `##QuoteExpiryDate##`, `##QuoteLink##`, `##QuotePDFURL##` are **not** on Quoter's published token list — dropped from the salvaged version rather than tested.

**The finding that matters: `Aim Games-3097` is now client-facing.** The per-deal org naming of §3.4 has been internal until now; the cover page puts it in front of a customer, twice.

**[Open]** `##CustomerOrganization##` reads the contact's `billing_organization`, which is also what the Quoter Client resolves from — one value, two audiences. Stripping the suffix would fix the cover page but would also merge every Aim Games deal into one Client, which is a behavioural change (§3.5), not a cosmetic one. Simplest option that needs no code: drop the token from the cover page and let the Prepared For block carry the account identifier, where a suffix matters less.

---

## 18. The Legacy Path Is Gone — **[Confirmed live, 2026-08-30]**

Done the same day §17 was written, so §17.5's "can be retired" is now past tense.

### 18.1 What was removed

One contiguous block, `quoter.py` line 1219 to EOF — **859 lines**:

| | |
| :-- | :-- |
| `add_template_line_items_to_quote` | walked `TEMPLATE_BUNDLES` |
| `add_default_instructional_item` | fallback for the above |
| `create_comprehensive_quote_from_pipedrive` | the legacy entry point |
| `create_comprehensive_quote_with_bundles` | already dead |

Plus the import at line 8.

**They came out in one piece because they only called each other.** `add_default_instructional_item` was called from 1892/1895 and 2060/2063 — all four inside the two composition functions. Nothing outside the block reached into it, which is what made a single cut safe rather than three excisions.

**Then archived:** `template_mapping_enhanced.py`, `verify_bundles.py`, `create_draft_quote.py` — moved with `git mv`, not deleted, so history follows.

**A naming trap worth recording.** `create_draft_quote.py` is a standalone script; `create_draft_quote` is also a **function in `quoter.py`** that `webhook_handler.py` imports and which survives. A grep for the name returns both. Check which before removing anything.

### 18.2 The flag is gone too

`USE_V2_COMPOSITION` existed so v2 could be deployed without committing to it. §17.1 established that rollback no longer works. So the branch collapsed to a direct call, and the variable was removed from Render.

**A flag that cannot be flipped is worse than no flag**, because someone will eventually trust it.

`test_render_webhook.sh` still advises checking the logs for `USE_V2_COMPOSITION=true` to confirm which path ran. **That advice is now stale** — there is one path, so a success is unambiguous.

### 18.3 Confirmed live

`python3 quote_composer.py --deal 3101` → 3 sections, 10 line items, unchanged. Then through the deployed webhook: `quot_3IeOomMoUVJeW2YZLULz33wwuGf`, and again after the archive move as `quot_3IeQDSvMcQ4tNKywwqgZmdsR3O4`. Sections verified in Quoter.

### 18.4 Three days of failed deploys, and the cause

**Production had been running Aug 28 code since the 28th.** Three consecutive deploys failed with **status 127** — "command not found".

The cause: commit `34154b7` on 2026-08-29 **removed `gunicorn==23.0.0` from `requirements.txt`** and reverted `render.yaml`'s start command. So Render ran `gunicorn ...` with gunicorn not installed.

**How it happened, and this is the part worth remembering.** The gunicorn change was made on the Mini on the 28th and pushed. The Air's working copy was older. `sync.sh` runs `git add -A` and commits whatever is on disk — **git cannot distinguish a deliberate revert from a stale file**, so an older `requirements.txt` was committed over a newer one and looked like an intentional edit.

**The likely mechanism is worse than a missed pull.** An `rclone copy` from Drive into the repo was run on the 29th to recover files. Drive's copies of `requirements.txt` and `render.yaml` dated from the 27th — rclone preserves mtime, and Drive was serving files older than the repo.

**So the hazard is not that Drive loses files. It is that Drive supplies stale ones back into a Git working tree.** Two mitigations, either sufficient:

- **never `rclone copy` into a Git working directory** — pull to a scratch folder and move deliberately
- **`git status` immediately after any rclone copy**, before `sync.sh`

Also confirmed by this: **Render runs Python 3.13.4 while `render.yaml` declares 3.14.6** — further evidence the blueprint is inert (§16.4).

---

## 19. Two Webhook Entry Points, Not One — **[Confirmed, 2026-08-30]**

Chapter 3 has described quote creation as having a single trigger. It has two, by design.

### 19.1 `2C-V1 Manual SubOrg/SubCust Repair`

**Trigger:** deal field `Run 2C Manual SubCust Repair` (field 100) changes to **Yes**. Changing it back to No does not re-run, because that fails the condition.

**Purpose:** completes Pipedrive and quote processing for an existing suborg **after** its QBO sub-customer has been created by a manual SyncQ Yes→No→Yes toggle. It does not attempt to trigger SyncQ itself.

It validates that the suborg has a QuickBooks ID, sync status Success, and a parent organization; sets `HID-QBO-Status = QBO-SubCust`; writes the QBO ids back to the deal; resets its own trigger field; **then fires the quote-creation webhook** at step 9.

So the handler cannot distinguish 2B from 2C, and should not need to — both arrive having satisfied the readiness gate.

The established division: **2A** creates the QBO mothership · **2B** the normal stage-based path · **2C** the repair path.

### 19.2 It is a clone, and its payload is identical

`2C-V1` was created by cloning `2B-V3` and deleting the earlier steps, so **step 9 carries the same 17 keys in the same order** as `2B-V3`'s step 19. Verified field by field.

**Any payload change is therefore one edit applied twice.** Both point at the same webhook definition, `Ready-Quoter-Draft Quote Creation-v3`, so an auth change is a single edit covering both.

### 19.3 The payload, and how little of it is used
...
**Field 102 is absent**, which is why the v2 branch re-fetches (§15.2).

`create_quote_v2` reads four: `{{deal.id}}` to re-fetch, `{{organization.name}}` for `client_name`, `{{person.email}}` for the contact, and the handler's gate uses `{{organization.id}}` plus HID-QBO-Status. The remaining twelve feed `_contact_from_webhook()` for address — and `{{person.phones}}` is not passed through at all.

**Slimming is blocked on the code, not on Pipedrive.** `_contact_from_webhook()` reads name, email and address from the payload rather than from the re-fetched deal. Change that first and the payload can drop to four keys; delete the keys first and contact resolution breaks.

### 19.4 The v2 webhook definition is safe to leave

`Ready-Quoter-Draft Quote Creation-v2` still exists, carries **no token**, and is referenced by four automations — `2B-V2`, its copy, `2C-V3` and `Test Webhook`. **All four are confirmed inactive**, so there is no 401 risk. Deleting it is optional; leaving it costs nothing and avoids breaking anything ever revived.

---


## 20. Open Questions for ScalePad / Jon Turner
...
- **[Done 2026-08-27]** ~~Wire the webhook.~~ Branch added, deployed, and confirmed live end to end (§15).
- **[Done 2026-08-30]** ~~Retire `template_mapping_enhanced.py`.~~ Done, with the whole legacy path (§18).
- **Fix `test_render_webhook.sh`** — it still advises checking logs for a flag that no longer exists (§18.2).
- **Slim the webhook payload** — 17 keys, 4 used. Blocked on `_contact_from_webhook()` reading the payload rather than the re-fetched deal (§19.3). Two automations to edit, identical payloads.
- ~~**Retire `template_mapping_enhanced.py`**~~ (§17.5). Save the cover letter text first — done, `docs/COVER_LETTER_SALVAGE_2026-08-30.md`. Then delete the file, retire `verify_bundles.py`, and remove the orphaned calls at `quoter.py:1748` and `:2003`.
- **Migrate `template_selection_logic.py` to `option_map(90)`** (§17.1). This is what restores a rollback; until then `USE_V2_COMPOSITION=false` breaks rather than reverts.
- **Decide the org-name leak** (§17.7). `Aim Games-3097` is now on a client-facing cover page.
- **Move webhook auth to HTTP Basic and rotate `WEBHOOK_SECRET`** (§16.5). Pipedrive supports Basic Auth but not custom headers, so the `X-Webhook-Token` route is a dead end for that caller. Six steps, and the callers must be updated before Render.
- **[Done 2026-08-28]** ~~Demote the retry 404 to a warning.~~ Done via `expect_statuses` (§16.3).
- **[Done 2026-08-28]** ~~Reconcile `render.yaml`.~~ Corrected, and confirmed inert (§16.4).
- **Confirm `Ready-Quoter-Draft Quote Creation-v2` is dead** before removing it — 4 automations still reference it and it carries no token (§16.5).
- **Review the Pipedrive automation.** With the re-fetch in place (§15.2) most of the payload is redundant for the v2 path; the deal id alone would do. Do not slim it until the legacy path is retired, since that path still needs the fat payload.
