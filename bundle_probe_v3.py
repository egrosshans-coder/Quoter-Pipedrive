...
def shapes(item_id, name, category, price):
    ...
    Docs for POST /quotes/{id}/sections/{sid}/line-items say validation
    errors are located as line_items[i].field, so the body is an ARRAY under
    a line_items key. v1 posted a flat object, which is why it got
    400 ERR_REQUEST_FORMAT_INVALID -- that was the wrapper, not the bundle.

    Note POST /line-items (singular) takes NO item_id: category, name,
    quantity, quote_id are required and the rest are values. So line items
    may be created BY VALUE rather than by reference. Both are tried.
    """
    by_ref = {"item_id": item_id, "quantity": 1}
    by_val = {"name": name, "category": category,
              "quantity": 1, "unit_price": price or 0}
    return [
        ("line_items[] by ref",   {"line_items": [by_ref]}),
        ("line_items[] by value", {"line_items": [by_val]}),
        ("line_items[] ref+qty_dec",
                                  {"line_items": [{"item_id": item_id,
                                                   "quantity_decimal": "1"}]}),
        ("bare array",            [by_ref]),
        ("flat by ref",           by_ref),
        ("flat by value",         by_val),
    ]
